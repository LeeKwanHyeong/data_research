...
class DefaultAdapter:
    """
    - 모델 forward 시그니처를 점검하여 지원하는 키워드만 전달
    - (x, future_exo, mode) 조합을 무작정 시도하지 않고, 지원 여부에 따라 안전 호출
    - RevIN denorm 지원(옵션)
    """

    # -------------------- utils --------------------
    def _call_model(self, model, x, *, future_exo=None, mode=None):
        accepts_exo = _model_accepts_kw(model, "future_exo")
        accepts_mode = _model_accepts_kw(model, "mode")

        kwargs = {}
        if future_exo is not None and accepts_exo:
            kwargs["future_exo"] = future_exo
        if mode is not None and accepts_mode:
            kwargs["mode"] = mode

        # 단 한 번의 호출만 시도: 지원하는 인자만 전달
    ...
    def forward(self, model, x_batch, *, future_exo=None, mode=None):
        ...
        # tuple/list 입력: 직접 언패킹 시도 -> 실패 시 (x, exo) 패턴 감지 -> 아니면 첫 요소만
        if isinstance(x_batch, (tuple, list)):
            try:
                return model(*x_batch)
            except TypeError:
